...
const ListMenuParent = () => {
  ...
  const handleClickCreate = (val) => {
    history.push(`/administrator/menu_parent/create`);
  };
  const handleClickRead = (item) => {
    history.push(`/administrator/menu_parent/read/${item.menu_parent_id}`);
  };
  const handleClickUpdate = (item) => {
    history.push(`/administrator/menu_parent/update/${item.menu_parent_id}`);
  };
