...
const ListMenu = () => {
  ...
  useEffect(() => {
    $axios
      .get(`resto/menu?page=${pagination.page}&limit=${pagination.size}`)
      .then((res) => {
  ...
  const handleClickRead = (item) => {
    history.push(`/${location[1]}/${location[2]}/read/${item.resto_menu_id}`);
  };
  const handleClickUpdate = (item) => {
    history.push(`/${location[1]}/${location[2]}/update/${item.resto_menu_id}`);
  };
    ...
    confirmAlert({
      ...
      buttons: [
        ...
        },
        {
          ...
          onClick: () => {
            var body = {
              data: { resto_menu_id: item.resto_menu_id },
            };
            console.log(body);
            $axios.delete(`resto/menu`, body).then((res) => {
  ...
  const fields = [
    { key: "resto_menu_id", label: "ID" },
    { key: "resto_menu_name", label: "Name" },
    { key: "resto_menu_category_name", label: "Category" },
    { key: "total_variant", label: "Total Variant" },
